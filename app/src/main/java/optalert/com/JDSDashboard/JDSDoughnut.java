...
public class JDSDoughnut extends Speedometer {
    private final static String TAG = "JDSDoughnut";
    ...
    private boolean doughnutVisibility = true;

    private float MEDIUM_ALERT = 4.5f;
    private float HIGH_ALERT = 5.0f;

    public JDSDoughnut(Context context) {
        this(context, null);
    }

    public JDSDoughnut(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public JDSDoughnut(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected SpeedometerDefault getSpeedometerDefault() {
        ...
        speedometerDefault.lowSpeedColor = Color.GREEN;
    ...
    private void updateEmboss() {
        ...
        EmbossMaskFilter embossMaskFilter = new EmbossMaskFilter(
                new float[] { 1f, 1f, 1f }, .6f, 3f, pxTOdp(getSpeedometerWidth())*.35f);
        tubePaint.setMaskFilter(embossMaskFilter);
        EmbossMaskFilter embossMaskFilterBac = new EmbossMaskFilter(
                new float[] { -1f, -1f, 0f }, .6f, 3f, pxTOdp(getSpeedometerWidth())*.35f);
    ...
    private void updateTubePaint()
    {
        //Log.d(TAG, "getSpeed(): speed=" + getSpeed());
        if (getSpeed() < MEDIUM_ALERT)
            tubePaint.setColor(getLowSpeedColor());
        else if (getSpeed() < HIGH_ALERT)
            tubePaint.setColor(getMediumSpeedColor());
        else
            tubePaint.setColor(getHighSpeedColor());
    }
    ...
    public void setDoughnutVisibility(boolean visibility) {
        doughnutVisibility = visibility;
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (doughnutVisibility) canvas.drawArc(speedometerRect,  getStartDegree(), sweepAngle, false, tubePaint);

        if (textVisibility) drawSpeedUnitText(canvas);
